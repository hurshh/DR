...
const List<_SmellQ> _items = [
  _SmellQ('Lavender', '💜', 'Floral',  Color(0xFFEDE7F6)),
  _SmellQ('Garlic',   '🧄', 'Food',    Color(0xFFFFF3E0)),
  _SmellQ('Grass',    '🌾', 'Nature',  Color(0xFFF1F8E9)),
  _SmellQ('Soap',     '🧼', 'Fresh',   Color(0xFFE0F7FA)),
];
...
class SmellSorterQuestion2Screen extends StatefulWidget {
  const SmellSorterQuestion2Screen({super.key});

  @override
  State<SmellSorterQuestion2Screen> createState() =>
      _SmellSorterQuestion2ScreenState();
}

class _SmellSorterQuestion2ScreenState
    extends State<SmellSorterQuestion2Screen> {
  ...
  void initState() {
    ...
    Future.delayed(const Duration(milliseconds: 400), () {
      AudioService.instance.speak(
        'Round 2! Keep sorting those smells into the right category!',
      );
    });
  ...
  void _onNext() {
    ...
    if (_index >= _items.length - 1) {
      ...
      Navigator.pushReplacementNamed(
        context,
        Routes.smellSorterQuestion3,
        arguments: GameCarry(
          scoreSoFar: 0,
          correctSoFar: (carry?.correctSoFar ?? 0) + _score,
          totalSoFar: (carry?.totalSoFar ?? 0) + _items.length,
          ...
          Row(
            children: [
              ...
              Container(
                ...
                child: Text('Round 2 · ${_index + 1}/${_items.length}',
                    style: const TextStyle(
                        fontSize: 13,
