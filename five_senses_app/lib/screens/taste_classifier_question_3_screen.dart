...
const List<_FoodItem> _foods = [
  _FoodItem(
    name: 'Pretzel', emoji: '🥨', correctTaste: 'Salty', cardBg: Color(0xFFFBE9E7),
    funFact: 'Pretzels are dipped in salty water before baking — SALTY to the core!',
  ),
  _FoodItem(
    name: 'Dark Chocolate', emoji: '🍫', correctTaste: 'Bitter', cardBg: Color(0xFFEDE7F6),
    funFact: 'More cocoa = more BITTER. Dark chocolate is 70%+ cocoa!',
  ),
  _FoodItem(
    name: 'Honey', emoji: '🍯', correctTaste: 'Sweet', cardBg: Color(0xFFFFF3E0),
    funFact: 'Honey is one of the SWEETEST natural foods — made by bees! 🐝',
  ),
  _FoodItem(
    name: 'Soy Sauce', emoji: '🫙', correctTaste: 'Salty', cardBg: Color(0xFFE8F5E9),
...
class TasteClassifierQuestion3Screen extends StatefulWidget {
  const TasteClassifierQuestion3Screen({super.key});

  @override
  State<TasteClassifierQuestion3Screen> createState() =>
      _TasteClassifierQuestion3ScreenState();
}

class _TasteClassifierQuestion3ScreenState
    extends State<TasteClassifierQuestion3Screen> {
  ...
  void initState() {
    ...
    Future.delayed(const Duration(milliseconds: 400), () {
      AudioService.instance.speak(
        'Round 3! You\'re doing great. One more practice round before the challenge!',
      );
    });
  }
  ...
  void _onNext() {
    if (_lives <= 0 || _currentIndex >= _foods.length - 1) {
      ...
      Navigator.pushReplacementNamed(
        context,
        Routes.tasteClassifierQuestion4,
        arguments: GameCarry(
              ...
              child: Center(
                child: ConstrainedBox(
                  ...
                  child: SingleChildScrollView(
                    padding: EdgeInsets.fromLTRB(isTablet ? 24 : 18, 16, isTablet ? 24 : 18, 24),
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('How does this food taste?',
                            style: TextStyle(fontSize: isTablet ? 22 : 18, fontWeight: FontWeight.w900, color: const Color(0xFF2A2A2A))),
                        const SizedBox(height: 12),
                        _buildFoodCard(food, isTablet),
                        const SizedBox(height: 12),
                        Text('Tap the taste it belongs to 👇',
                            style: TextStyle(fontWeight: FontWeight.w600, color: const Color(0xFFB7B7B7), fontSize: isTablet ? 17 : 15)),
                        const SizedBox(height: 12),
        ...
        children: [
          Padding(
            ...
            child: Row(
              children: [
                IconButton(
                  onPressed: () => Navigator.pushReplacementNamed(context, Routes.tasteClassifierQuestion2),
                  icon: const Icon(Icons.arrow_back_ios_rounded, color: Colors.white, size: 22),
                  padding: EdgeInsets.zero, constraints: const BoxConstraints(),
                ),
                const SizedBox(width: 6),
                Text('Taste Classifier',
                    style: TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: isTablet ? 26 : 20)),
                const Spacer(),
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                  decoration: BoxDecoration(color: Colors.white24, borderRadius: BorderRadius.circular(20)),
                  child: const Text('Round 3 / 4',
          ...
          Positioned(
            ...
            child: Text('Food ${_currentIndex + 1} of ${_foods.length}',
                style: TextStyle(color: Colors.white.withValues(alpha: 0.9), fontWeight: FontWeight.w700, fontSize: isTablet ? 16 : 14)),
          ),
          Positioned(
            left: 0, right: 0, bottom: 0,
            child: LinearProgressIndicator(
              value: (_currentIndex + 1) / _foods.length, minHeight: 5,
  ...
  Widget _buildFoodCard(_FoodItem food, bool isTablet) {
    return Container(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Text(food.name, style: TextStyle(fontSize: isTablet ? 26 : 20, fontWeight: FontWeight.w900, color: const Color(0xFF2A2A2A))),
      ]),
    );
  }
  ...
  Widget _buildNextButton(bool isTablet) {
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        ...
        child: Text(isLast ? 'Final Challenge →' : 'Next Food →',
